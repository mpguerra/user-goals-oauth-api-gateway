-- 0483ba5b23323f959066b6de87912347 http://api.2445580413231.proxy.3scale.net:80 https://sentiment-api.herokuapp.com:443 2555417688881 sentiment-api.herokuapp.com --
-- -*- mode: lua; -*-
-- Generated on: 2014-01-22 10:08:43 +0000 --
-- Version:
-- Error Messages per service

      service_2555417688881 = {
...
matched_rules2 = ""
        ...
        function extract_usage_2555417688881(request)
        ...
        local args = get_auth_params(nil, method)
             local m =  ngx.re.match(path,[=[^/v1/words/([\w_\.-]+)\.json]=])
     if (m and method == "GET") then
     -- rule: /v1/words/{word}.json --
              params["word"] = m[1]
         table.insert(matched_rules, "/v1/words/{word}.json")

         usage_t["get_word"] = set_or_inc(usage_t, "get_word", 1)
         found = true
          end

     local m =  ngx.re.match(path,[=[^/v1/sentences/([\w_\.-]+)\.json]=])
     if (m and method == "GET") then
     -- rule: /v1/sentences/{sentence}.json --
              params["sentence"] = m[1]
         table.insert(matched_rules, "/v1/sentences/{sentence}.json")

         usage_t["get_sentence"] = set_or_inc(usage_t, "get_sentence", 1)
         found = true
          end
     ...
     -- rule: /v1/words/{word}.json --
         ...
         usage_t["set_word"] = set_or_inc(usage_t, "set_word", 1)
         found = true
          end
...
function get_debug_value()
   ...
   if h["X-3scale-debug"] == '0483ba5b23323f959066b6de87912347' then
      return true
   else
      return false
   end
...
ngx.log(0,"In nginx_3scale_access.lua")
local params = {}
local host = ngx.req.get_headers()["Host"]
local auth_strat = ""
local service = {}
      if ngx.var.service_id == '2555417688881' then
        local parameters = get_auth_params("not_headers", string.split(ngx.var.request, " ")[1] )
        service = service_2555417688881 --
        ngx.var.secret_token = service.secret_token
        params.access_token = parameters["access_token"]

        get_credentials_access_token(params, service_2555417688881)
-- TODO: ensure caching is on
        --ngx.var.cached_key = "2555417688881" .. ":" .. params.app_id ..":".. params.app_key
              auth_strat = "oauth"
        ngx.var.service_id = "2555417688881"
        ngx.var.proxy_pass = "https://backend_sentiment-api.herokuapp.com"
        ngx.var.usage = extract_usage_2555417688881(ngx.var.request)
      end
